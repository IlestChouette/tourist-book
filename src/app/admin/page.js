  ...
  return (
    <main className="flex-1 bg-sand">
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        <h1 className="font-display italic text-3xl text-ink">Resumen</h1>
        <div className="mt-6 grid gap-4 sm:grid-cols-4">
          <Stat label="Clientes" value={hosts?.length ?? 0} />
          <Stat label="Alojamientos activos" value={activeProperties.length} />
          <Stat label="MRR estimado" value={`${mrr.toFixed(2)} €`} />
          <Stat label="ARR estimado" value={`${(mrr * 12).toFixed(0)} €`} />
        </div>

        <h2 className="mt-12 font-display italic text-2xl text-ink">
          Solicitudes de cancelación {pendingRequests.length > 0 && `(${pendingRequests.length} pendientes)`}
        </h2>
        <div className="mt-4 grid gap-3">
          {(requests ?? []).length === 0 && <p className="text-ink/60">No hay solicitudes.</p>}
          {(requests ?? []).map((r) => (
            <div key={r.id} className="rounded border border-sand-dim bg-sand-card p-4">
              ...
              <p className="mt-2 text-xs text-ink/50">{new Date(r.created_at).toLocaleString("es-ES")}</p>
              {r.status === "pendiente" && (
        ...
        <h2 className="mt-12 font-display italic text-2xl text-ink">Alojamientos</h2>
          ...
          <table className="w-full min-w-[720px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-sand-dim bg-sand-card text-left">
                <th className="px-4 py-2 font-bold text-ink/70">Alojamiento</th>
                <th className="px-4 py-2 font-bold text-ink/70">Hotelero</th>
                <th className="px-4 py-2 font-bold text-ink/70">Plan</th>
                <th className="px-4 py-2 font-bold text-ink/70">Ciclo</th>
                <th className="px-4 py-2 font-bold text-ink/70">Estado</th>
              </tr>
            </thead>
            <tbody>
              {(properties ?? []).map((p) => (
                <tr key={p.id} className="border-b border-sand-dim last:border-0">
                  ...
                  <td className="px-4 py-2 text-ink/70">{statusLabel[p.subscription_status] ?? p.subscription_status ?? "sin suscripción"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <h2 className="mt-12 font-display italic text-2xl text-ink">Clientes</h2>
        <div className="mt-4 overflow-x-auto rounded border border-sand-dim">
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-sand-dim bg-sand-card text-left">
                <th className="px-4 py-2 font-bold text-ink/70">Nombre</th>
                <th className="px-4 py-2 font-bold text-ink/70">Email</th>
                <th className="px-4 py-2 font-bold text-ink/70">Alta</th>
              </tr>
            </thead>
            <tbody>
              {(hosts ?? []).map((h) => (
                <tr key={h.id} className="border-b border-sand-dim last:border-0">
                  ...
                  <td className="px-4 py-2 text-ink/70">{new Date(h.created_at).toLocaleDateString("es-ES")}</td>
                </tr>
              ))}
            </tbody>
          </table>
